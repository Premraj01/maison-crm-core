...
interface CrmContextValue { theme:Theme; setTheme:(theme:Theme)=>void; role:Role; railCollapsed:boolean; setRailCollapsed:(v:boolean)=>void; leads:Lead[]; leadsStatus:"loading"|"ready"|"error"; leadsError:string|null; setLeadStage:(id:string,stage:LeadStage,value?:number)=>Promise<void>; upsertLead:(lead:Lead)=>void; reloadLeads:()=>void; agents:ApiUser[]; properties:Property[]; propertiesStatus:"loading"|"ready"|"error"; propertiesError:string|null; upsertProperty:(property:Property)=>void; reloadProperties:()=>void; tasks:typeof seedTasks; toggleTask:(id:string)=>void }
...
export function CrmProvider({children}:{children:ReactNode}) {
  ...
  // Properties are the one entity already backed by Postgres, so they are
  // fetched rather than seeded — and the public website reads the same rows.
  ...
  const {user,token}=useAuth(); const role:Role=user?.role ?? "viewer";
  ...
  // Reads are public on the API, so this runs whether or not a session has
  // been restored yet.
  useEffect(()=>{ let cancelled=false; setPropertiesStatus("loading");
    fetchProperties()
  ...
  const value=useMemo(()=>({theme,setTheme,role,railCollapsed,setRailCollapsed,leads,leadsStatus,leadsError,setLeadStage:async(id:string,stage:LeadStage,value?:number)=>{
    ...
    },upsertLead:(lead:Lead)=>setLeads(v=>{const exists=v.some(l=>l.id===lead.id);return exists?v.map(l=>l.id===lead.id?lead:l):[lead,...v]}),reloadLeads:()=>setLeadsNonce(n=>n+1),agents,properties,propertiesStatus,propertiesError,upsertProperty:(property:Property)=>setProperties(v=>{const exists=v.some(p=>p.id===property.id);return exists?v.map(p=>p.id===property.id?property:p):[property,...v]}),reloadProperties:()=>setPropertiesNonce(n=>n+1),tasks,toggleTask:(id:string)=>setTasks(v=>v.map(t=>t.id===id?{...t,done:!t.done}:t))}),[theme,role,railCollapsed,agents,leads,leadsStatus,leadsError,token,properties,propertiesStatus,propertiesError,tasks]);
