...
interface CrmContextValue { theme:Theme; setTheme:(theme:Theme)=>void; role:Role; setRole:(role:Role)=>void; railCollapsed:boolean; setRailCollapsed:(v:boolean)=>void; leads:Lead[]; setLeadStage:(id:string,stage:LeadStage)=>void; upsertLead:(lead:Lead)=>void; tasks:typeof seedTasks; toggleTask:(id:string)=>void }
const CrmContext = createContext<CrmContextValue | null>(null);
export function CrmProvider({children}:{children:ReactNode}) {
  const [theme,setThemeState]=useState<Theme>("light"); const [role,setRole]=useState<Role>("admin");
