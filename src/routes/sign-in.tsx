...
export const Route = createFileRoute("/sign-in")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Sign in to the Maison client relationship workspace." },
    ...
    <main className="grid min-h-screen bg-background lg:grid-cols-2">
      <section className="relative hidden overflow-hidden border-r border-border bg-secondary lg:block">
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-secondary via-secondary/90 to-transparent p-10 pt-24">
          ...
          <p className="mt-4 max-w-md text-sm leading-6 text-muted-foreground">
            A quieter way to manage the people, conversations and opportunities that move your work forward.
          </p>
        </div>
      </section>
      <section className="flex items-center justify-center p-6">
        <div className="page-enter w-full max-w-sm">
          ...
          <p className="text-[10px] font-semibold uppercase tracking-[.22em] text-primary">Welcome back</p>
          <h1 className="mt-3 font-display text-4xl">Sign in to Maison</h1>
          <p className="mt-2 text-sm text-muted-foreground">Use any details to preview the workspace.</p>
          <form className="mt-8 space-y-5" onSubmit={(e) => e.preventDefault()}>
            ...
            <Button asChild size="lg" className="w-full">
              <Link to="/">Sign in <ArrowRight /></Link>
            </Button>
          </form>
          <p className="mt-6 text-center text-sm text-muted-foreground">
            New to Maison? <Link to="/register" className="font-semibold text-primary">Create an account</Link>
          </p>
          <p className="mt-6 text-center text-xs text-muted-foreground">Visual preview only · no account required</p>
        </div>
      </section>
    </main>
  );
